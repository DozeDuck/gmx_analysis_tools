...
class plotly_go():
    ...
    def __init__(self,file1, file2, file3, output_name, renumber, ave):
        ...
        if self.flag == 'rmsd':
            ...
            self.rmsf_averager(file1, file2,file3, renumber, output_name)
    ...
    def rmsd_averager(self, file1, file2, file3, ave, output_name):
        ...
        x_name = 'Time (ns)'
        y_name = 'RMSD (nm)'
        self.plotly_noSD(plot_title, x_name, y_name, self.time1, self.values1, self.values2, self.values3)     

        

                    
    def rmsf_averager(self, file1, file2, file3, renumber, output_name):
        a = 17
        with open(file1, 'r') as f:
            lines = f.readlines() 
            for i in range(len(lines)):
    ...
    def sasa_res_averager(self, file1, file2, file3,renumber,ave, output_name):
        ...
        x_name = 'Residue Number'
        y_name = 'SASA Area (nm<sup>2</sup>)'
        self.plotly_noSD(plot_title_noSD, x_name, y_name, self.time1, self.values1, self.values2, self.values3)
        self.plotly_SD(plot_title_SD, x_name, y_name, self.time1, self.values1, self.values2, self.values3, self.sd1, self.sd2, self.sd3)  
    ...
    def plotly_noSD(self, plot_title, x_name, y_name, time1, values1, values2, values3):
    ...
    def plotly_SD(self, plot_title, x_name, y_name, time1, values1, values2, values3, sd1, sd2, sd3):
            trace1 = go.Scatter(x=time1, y=values1, name='Replica 1')
