...
try:
    ...
    print('Usage: ./plotly_go -f <file1> -s <file2> -t <file3> -o <output_name> -r <true/fault> -a <2/3> \
          \nCurrently it works for rmsd, rmsf, sasa_time, sasa_residue, gyration, dipole movement  \
...
class plotly_go():
    ...
    def __init__(self,file1, file2, file3, output_name, renumber, ave):
        ...
        if self.flag == 'rmsd':
            ...
            self.rmsf_averager(file1, file2,file3, renumber, ave, output_name)
        elif self.flag == 'sasa' and self.sasa_flag != '-or':
            self.sasa_averager(file1, file2, file3, ave, output_name)
        elif self.flag == 'sasa' and self.sasa_flag == '-or':
    ...
    def rmsf_averager(self, file1, file2, file3, renumber, ave, output_name):
        a = 17
        with open(file1, 'r') as f:
            lines = f.readlines() 
            for i in range(len(lines)):
    ...
    def sasa_averager(self, file1, file2, file3, ave, output_name):
        a = 24
        with open(file1, 'r') as f:
            lines = f.readlines() 
            for i in range(len(lines)):
